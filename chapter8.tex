...
%that we should have more randomized initial structures, a rectangle, a cube or a disc are too sharp to be used as an initial structure of a flock. 
In addition, real boids have a limited field of view. 
%since they are alive entities. 
...
%Not all the time we are going to have the same amount of entities, something can cause an entity to die or just to go away.
...
%Engine. By developing the implementations of all that was stated in the above Section, we are going to be close to have a complete Boids system inside the Blender Game Engine.  

%The UI modifier has more options for the parameters than required. Those parameters have default values and the option to be computed by the optimization algorithms already mentioned i.e. PSO and GA, or they can be manually controlled by the user before the game starts.
