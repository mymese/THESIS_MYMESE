...
GPUs stand for \textit{Graphics Processing Units}, and they were original created for graphics, general imaging and games. GPUs been used lately for non-rendering purposes in which only intensive computation is needed. GPUs are mainly used for massively computation in the computers.  Two of the most used GPU programming languages are CUDA and OpenCL. Since flocking is a parallelizable algorithm, GPUs can be used to improve its performance.
...
% GPUs eveolves rapidly, almost every 6 months NVIDIA duplicate their current computational power on the GPUs, allowing graphics artists, game developers and scientists to improve their work in quality and quantity. 
...
% OpenCL - Open Computing Language; is a C-language extension; use for parallel programming of heterogeneous systems, i.e. GPUs and CPUs; initially proposed by Apple and now managed by Khronos Group
